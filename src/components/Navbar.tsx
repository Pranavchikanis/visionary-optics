          ...
          <div className="flex items-center space-x-6">
            ...
            {session ? (
              <div className="relative group">
                <div className="text-gray-500 hover:text-indigo-600 cursor-pointer flex items-center transition-colors">
                  <FiUser className="h-5 w-5 mr-1.5" />
                  <span className="text-sm font-medium hidden md:block">{session.user?.name?.split(' ')[0]}</span>
                </div>
                <div className="absolute right-0 w-48 mt-2 origin-top-right bg-white border border-gray-100 rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform group-hover:translate-y-0 translate-y-1">
                  <div className="py-2">
                    ...
                    <button onClick={() => signOut()} className="block w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 mt-1 border-t border-gray-100 pt-2">Sign out</button>
                  </div>
                </div>
              </div>
            ) : (
